...
namespace Data
{
   ...
   private:
      ...
      static void MakeRaceFormat(const CharacterRace& Race, Format::Race& Format)
      {
         ...
         //
         // Appendix         
         for (int StepArray = 0; StepArray < 20;) // Character.Race.AllowedClassIDs; // 16  * 20 bits : 320 bits, only store the first two bytes, we won't have 16k races  so we can get away with is
         {
            const int StepID = Race.AllowedClassIDs[StepArray++];
            Format[StepFormat++] = MASK_BYTE(StepID, 0);
            Format[StepFormat++] = MASK_BYTE(StepID, 1);
         }
         for (int StepArray = 0; StepArray < 6;) // Character.Race.Modifiers; // 8 * 6  bits : 48 bits // we need about 5 bits per modifier and we have 6 stats we can affect  
         {
            const int ModValue = Race.Modifiers[StepArray++];
            Format[StepFormat++] = MASK_BYTE(ModValue, 0);
         }
         for (int StepArray = 0; StepArray < 6;) // Character.Race.MinimumStats; // 8 * 6  bits : 48 bits // we need about 5 bits per min_stat and we have 6 stats we can affect
         {
      ...
      static void MakeCharacterFormat(const DnDCharacter& Character, Format::Char& ChrFormat)
      {
         ...
         ChrFormat[StepFormat++] = MASK_BYTE(Character.Hitpoints, 0); // Store only the first 16 bits of hitpoints, anything above is excessive
